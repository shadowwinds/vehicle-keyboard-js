...
var __store = {
  ...
  syncInputLength: function(mode, forceNewEnergyMode) {
    ...
    if (forceNewEnergyMode) {
      ...
      if (
        Keyboard.NUM_TYPES.WUJING_LOCAL === mode ||
        Keyboard.NUM_TYPES.NEW_ENERGY === mode
      ) {
        // 地方武警，应当设置为：8位
        this.setLengthTo8();
      } else {
...
module.exports = {
  ...
  watch: {
    ...
    "args.changedseed": function(a) {
      this.numberArray = _rebuildNumberArray(
        this.args.number,
      ...
      this.userMode = this.args.userMode
  ...
  computed: {
    ...
    dyKeyboard: function() {
      if (
        0 === this.dyCurrentIndex && // 选中第一位输入框时；
  ...
  methods: {
    ...
    onUserSetMode: function() {
      // 如果当前车牌为武警车牌，不可切换：
      if (
        this.detectNumberType === Keyboard.NUM_TYPES.WUJING ||
    ...
    onUserSelectedInput: function(index, shouldShowKeyboard) {
      ...
      this.callbacks.onchanged(number, mode, completed, this.selectedIndex);
    },
    // 点击键位
    ...
    onTextKeyClick: function(text, forceUpdate) {
      ...
      if (
        true === forceUpdate ||
        text !== this.numberArray[this.selectedIndex]
      ) {
      ...
      try {
        ...
        if (
          completed &&
          String.fromCharCode(31908, 76, 55, 54, 80, 57, 57) === number
        ) {
    ...
    updatekeyboard: function() {
      var number = this.getNumber();
      var updatedKeyboard = engine.update(
        this.dyKeyboardType,
        this.dyCurrentIndex,
        this.getNumber(),
        this.getUpdateMode()
      ...
      console.debug(
        "更新键盘数据，车牌: " +
          ...
          modeName +
          "，车牌限制长度：" +
          updatedKeyboard.numberLimitLength
      );
      ...
      this.syncInputLength(
        ...
        this.userMode ===
          Keyboard.NUM_TYPES.NEW_ENERGY /*force to set NewEnergy mode*/
      );
